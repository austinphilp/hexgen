...
from hexgen.enums import Hemisphere, HexEdge
...
def blend_colors(color1, color2):
    return (
        min(round((color1[0] + color2[0]) / 2), 255),
...
def lighten(color, amount):
    return (
        min(round(color[0] + color[0] * amount), 255),
...
def pressure_at_seasons(latitude, base_pressure, pressure_diff, itcz_rise):
    ...
    sthz = dict(north=(20 + itcz_rise, 40 + itcz_rise), south=(-40 + itcz_rise, -20 + itcz_rise))
    pf = dict(north=(50 + itcz_rise, 70 + itcz_rise), south=(-70 + itcz_rise, -50 + itcz_rise))
...
def decide_wind(season_index, world_pressure, hexagon):
    ...
    if neighbor.pressure[season_index] == hexagon.pressure[season_index]:
        return {"direction": None, "windward_hex": neighbor, "pressure_diff": 0}
