...
from hexgen.hex import HexFeature, HexSide
...
class MapGen:
        ...
        if self.params.get("volcanoes"):
            ...
            for volcano in volcanoes:
                ...
                for i in size_list:
                    ...
                    if i == 1:
                        hexes = center_hex.surrounding + [center_hex]
                    else:
                        hexes = center_hex.bubble(distance=i)
                    for hex in hexes:
                        hexes.append(hex)
    ...
    def _generate_pressure(self):
        ...
        with Timer("Generating wind", self.debug):
            for y, row in enumerate(self.hex_grid.grid):
                for x, col in enumerate(row):
                    h = self.hex_grid.grid[x][y]
                    h.wind = (decide_wind(0, base_pressure, h), decide_wind(1, base_pressure, h))
    ...
    def _generate_rivers(self):
        ...
        """
        num_rivers = self.params["num_rivers"]
        ...
        for r in self.rivers_sources:  # loop over each source segment
            ...
            while finished is False:
                ...
                if one_valid and two_valid:
                    ...
                    segment = segment.next
                # TODO - get the below code working to generate lakes
                else:
